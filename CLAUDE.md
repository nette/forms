...
- **Dependencies:** nette/component-model (^3.2 || ^4.0), nette/http (^3.4), nette/utils (^4.1)
- **Latte Integration:** Requires Latte 3.1.4+ (conflict with < 3.1.4 or >= 3.2)
...
- Choice controls: `SelectBox`, `MultiSelectBox`, `RadioList`, `CheckboxList`
- Special: `Button`, `SubmitButton`, `ImageButton`, `Checkbox`, `HiddenField`, `ColorPicker`, `DateTimeControl`, `UploadControl`, `CsrfProtection`
...
- Requires PHP 8.3 (was 8.1 in v3.2)
- Latte Runtime refactored from static to non-static class
- Removed Latte 2 support (requires Latte 3.1.4+)
- Compatibility with nette/component-model 4 (allows ^3.2 || ^4.0)
- CSRF protection now checks same-origin via the `Sec-Fetch-Site` header instead of SameSite cookie; token-based protection is deprecated
- `Container::setValues()` / `setDefaults()` now accept `iterable|stdClass` (BC break)
- `RadioList` / `CheckboxList`: `getControlPart()` and `getLabelPart()` throw an exception for an invalid key
...
- `phpstan.neon` - Static analysis (level 8, Nette extension)
