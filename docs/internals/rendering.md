...
so custom markup ultimately flows through those `Html` elements. Both also share
(by duplication) the **GET-form treatment**: the query string is stripped from
`action` at begin and its parameters are re-emitted as hidden inputs at end,
skipping names that collide with form components.
...
events **and resets every control's `rendered` option** — `renderFormEnd` (and the
default renderer's `renderEnd`) rely on that flag to emit not-yet-rendered hidden
fields before `</form>`; `end()` pops both. `get()` resolves an element from the
current scope and —
