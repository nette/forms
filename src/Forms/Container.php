...
class Container extends Nette\ComponentModel\Container implements \ArrayAccess
{
	...
	public function getValues(string|object|bool|null $returnType = null, ?array $controls = null): object|array
	{
		...
		if ($form && ($submitter = $form->isSubmitted())) {
			...
			if ($controls === null && $submitter instanceof SubmitterControl) {
				...
				$scopeControl = $this;
				while ($scopeControl->parent instanceof self && !is_null($controls) && !in_array($scopeControl, $controls)) {
					$scopeControl = $scopeControl->parent;
					if (in_array($scopeControl, $controls)) {
						$controls[] = $this;
						break;
					}
				}
	...
	public function validate(?array $controls = null): void
	{
		...
		foreach ($this->onValidate as $handler) {
			...
			$args = isset($types[0]) && !$types[0]->allows($this::class)
				? [$this->getUnsafeValues($types[0]->getSingleName())]
				: [$this, isset($params[1]) ? $this->getUnsafeValues($types[1]?->getSingleName()) : null];
			$handler(...$args);
		}
	}
	 ...
	 */
	public function addRadioList(string $name, string|Stringable|null $label = null, ?array $items = null): Controls\RadioList
	{
