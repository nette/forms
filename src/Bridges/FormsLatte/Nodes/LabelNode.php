	...
	public function print(PrintContext $context): string
	{
		return $context->format(
			($this->name instanceof StringNode
				? 'if ($ʟ_label = end($this->global->formsStack)[%raw]->'
				: '$ʟ_input = is_object($ʟ_tmp = %raw) ? $ʟ_tmp : end($this->global->formsStack)[$ʟ_tmp]; if ($ʟ_label = $ʟ_input->')
			. ($this->part ? ('getLabelPart(%raw)') : 'getLabel()')
			. ') echo $ʟ_label'
			. ($this->attributes->items ? '->addAttributes(%2.raw)' : '')
			. ($this->void ? ' %3.line;' : '->startTag() %3.line; %4.raw if ($ʟ_label) echo $ʟ_label->endTag() %5.line;'),
			$this->name,
			$this->part,
			$this->attributes,
			$this->position,
