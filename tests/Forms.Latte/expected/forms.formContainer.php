...
';
		$this->global->forms->begin($formContainer = $this->global->forms->item('cont1', Nette\Forms\Container::class)) /* pos %d%:2 */;
		echo '	<tr>
		<th>';
	...
	<tr>
		...
		<td>
';
		$this->global->forms->begin($formContainer = $this->global->forms->item('cont2', Nette\Forms\Container::class)) /* pos %d%:8 */;
		echo '			<ol>
';
...
';
		...
		$this->global->forms->end();
		$formContainer = $this->global->forms->current();
		$this->global->forms->begin($formContainer = $this->global->forms->item('items', Nette\Forms\Container::class)) /* pos %d%:2 */;
		echo '	<tr>
		<th>Items</th>
		<td>
';
		...
		foreach ($items as $item) /* pos %d%:3 */ {
			...
			$this->global->forms->begin($formContainer = $this->global->forms->item($item, Nette\Forms\Container::class)) /* pos %d%:4 */;
			echo '				';
